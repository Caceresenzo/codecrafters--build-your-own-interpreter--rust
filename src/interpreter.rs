...
impl Interpreter {
    ...
    pub fn resolve(&mut self, expression: &Expression, depth: u32) {
        let ptr = addr_of!(expression) as u64;
        println!("resolve {ptr} with depth {depth}");

        self.locals.insert(ptr, depth);
        println!("resolved {:?}", self.locals);
    }

    pub fn look_up_variable(&mut self, name: &Token, expression: &Expression) -> EvaluateInterpreterResult {
        let ptr = addr_of!(expression) as u64;

        if let Some(distance) = self.locals.get(&ptr) {
            println!("look_up_variable {ptr} with distance {distance}");
            self.environment.get_at(*distance, &name.lexeme)
        } else {
            println!("look_up_variable {ptr} with not found");
